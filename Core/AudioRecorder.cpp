...
void AudioRecorder::processBlock(const juce::AudioBuffer<float>& inputBuffer)
{
    ...
    if (circularBuffer.hasOverrun())
    {
        ...
        // Log overrun but don't spam the log
        static juce::uint32 lastOverrunLog = 0;
        if (juce::Time::getMillisecondCounter() - lastOverrunLog > 5000) // Log every 5 seconds max
        {
            logRecordingEvent("Warning: Recording buffer overrun detected");
            lastOverrunLog = juce::Time::getMillisecondCounter();
        }
    }
    ...
    if (recordedSeconds >= maxRecordingTimeSeconds)
    {
        ...
        logRecordingEvent("Recording auto-stopped at maximum duration");
    }
}
...
void AudioRecorder::stopRecording()
{
    ...
    // Give a moment for any final audio processing
...
juce::Array<juce::File> AudioRecorder::getRecentRecordings() const
{
    ...
    recordingDirectory.findChildFiles(files, juce::File::findFiles, false, "*.wav;*.aiff");
