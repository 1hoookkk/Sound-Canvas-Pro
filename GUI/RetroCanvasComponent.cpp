...
RetroCanvasComponent::RetroCanvasComponent()
{
    ...
    DBG("SpectralCanvas initialized - ready for modern audio painting!");
}
...
void RetroCanvasComponent::paint(juce::Graphics& g)
{
    ...
    g.fillAll(CanvasColors::CANVAS_IVORY);
    
    // Draw main canvas area
    g.setColour(CanvasColors::CANVAS_WHITE);
    g.fillRect(geom.canvasArea);
    
    // Draw terminal-style border around canvas
    drawTerminalBorder(g, geom.canvasArea, "SPECTRAL PAINT CANVAS", CanvasColors::EMERALD_GREEN);
...
{
    g.setColour(CanvasColors::CANVAS_IVORY.brighter(0.1f));
    ...
    g.setColour(CanvasColors::BRIGHT_ORANGE);
...
{
    return juce::Font(juce::FontOptions("Courier New", size, juce::Font::FontStyleFlags::plain))
...
void RetroCanvasComponent::beginPaintStroke(juce::Point<float> canvasPoint, float pressure)
{
    ...
    // Handle SampleBrush differently - trigger ForgeVoice samples
...
void RetroCanvasComponent::updatePaintStroke(juce::Point<float> canvasPoint, float pressure)
{
    ...
    // Handle SampleBrush differently - update pitch in real-time
    if (currentBrushType == BrushType::SampleBrush && processor)
    {
        processor->triggerPaintBrush(canvasPoint.y, pressure);
    }
...
void RetroCanvasComponent::setBrushType(BrushType type)
{
    ...
    switch (type)
    {
        case BrushType::SineBrush:     brushColor = CanvasColors::PAINT_BLUE; break;
        case BrushType::HarmonicBrush: brushColor = CanvasColors::BRIGHT_BLUE; break;
        case BrushType::NoiseBrush:    brushColor = CanvasColors::VIBRANT_RED; break;
        case BrushType::SampleBrush:   brushColor = CanvasColors::ROYAL_BLUE; break;
        case BrushType::GranularPen:   brushColor = CanvasColors::PURPLE; break;
        case BrushType::CDPMorph:      brushColor = CanvasColors::BRIGHT_ORANGE; break;
    }
